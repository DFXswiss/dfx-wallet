...
### Private payments (Cloister shielded pool)

Opt-in, KYC-gated privacy layer built on DFX's **Cloister** shielded-pool protocol: a payer
settles a payment request without revealing the on-chain payer↔recipient link, while proving in
zero knowledge that the funds belong to a compliance good-set. The Groth16 proof is generated
**on-device** (~0.3–0.4 s, native gnark); the witness never leaves the phone. Currently a **Base
Sepolia testnet pilot** — switched on per user in Settings → Private Payments (requires DFX KYC)
and configured via `EXPO_PUBLIC_CLOISTER_*`. Not behind an `EXPO_PUBLIC_ENABLE_*` flag; it is
inert unless a pool is configured **and** the user opts in.

| Function                                                                            | Technical components                                                                                                                               | Status                  | Tests                                                        |
| ----------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------- | ------------------------------------------------------------ |
| Private Payments opt-in (toggle + plain-language info + KYC gate + first-run popup) | `(auth)/private-payments.tsx`, `features/cloister/PrivatePaymentsScreenImpl.tsx`, `cloisterEnabled` setting, DFX KYC redirect                      | pilot — opt-in, testnet | partial (`test/services/cloister-extdata.test.ts`)           |
| Normal / Private payment toggle in the pay flow                                     | `(auth)/pay/cloister.tsx` (shielded path), `app/cloister-pay.tsx` (deep-link shim `dfxwallet://cloister-pay`), single-scan dedup                   | pilot — opt-in, testnet | none (E2E)                                                   |
| On-device shielded deposit (the private payment itself)                             | `features/cloister/deposit.ts`, `treeCache.ts`, `extData.ts`, `ownerKey.ts`, `store/cloister-tx.ts`; ethers/WDK submit + key-less relayer fallback | pilot — opt-in, testnet | partial (`cloister-extdata.test.ts` cross-stack extData KAT) |
| Native ZK prover (gnark Groth16 / Poseidon2)                                        | `modules/cloister-prover/` — Expo module wrapping `Cloister.xcframework` (fetched + sha256-verified at `pod install`), proving/verifying keys      | pilot — iOS, testnet    | none                                                         |
...
| `EXPO_PUBLIC_CLOISTER_POOL` / `_CHAINID` / `_RPC` / `_RELAYER` / `_FROM_BLOCK` / `_GETLOGS_RPCS` | Cloister shielded-pool config (pool address, chain, RPC, key-less relayer URL, deploy block, log-scan RPC list). No pool → private payments stay inert                          | unset (default chain 84532 = Base Sepolia) | partial (`cloister-extdata.test.ts`)                          |
| `EXPO_PUBLIC_CLOISTER_KEY` / `_EVAL` / `_SIDELOAD` / `_LAN`                                      | **Pilot-only** flags: embedded testnet deployer key, eval bypass, sideload auto-auth, LAN dev relayer. A release guard fails the build if any are set on mainnet (chainId 8453) | unset                                      | partial (`scripts/cloister-release-guard.js`)                 |
