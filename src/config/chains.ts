...
const buildAllNetworks = (): WdkConfigs['networks'] => ({
  ethereum: {
    blockchain: 'ethereum',
    config: {
      chainId: 1,
...
// WDK initializes every configured network on first wallet setup, each with its
// own RPC/bundler round-trips. Production wants all of them; the E2E/testnet
// build sets EXPO_PUBLIC_WDK_CHAINS to a minimal allowlist so onboarding init
// finishes well inside the visual-regression timeout instead of stalling on a
// fresh CI simulator (see docs/visual-regression.md).
export const getWdkConfigs = (): WdkConfigs => {
  ...
  const allow = process.env.EXPO_PUBLIC_WDK_CHAINS?.split(',')
    ...
    .filter(Boolean);
  if (!allow?.length) return { networks };
